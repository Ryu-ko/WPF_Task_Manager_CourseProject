...
namespace MizuFlow
{
    public partial class ToDoSubItem : UserControl
    {
        ...
        {
            subtask = MizuFlow.Registration.unit.Subtasks.Get(s => s.ID_Subtask == id).Single();
            attach.TA_SubtaskID = subtask.ID_Subtask;
            ...
            if (MizuFlow.Registration.unit.Attachments.Get(a => a.TA_SubtaskID == subtask.ID_Subtask && a.TA_TaskID == null).Count() != 0)
            {
                attach = MizuFlow.Registration.unit.Attachments.Get(a => a.TA_SubtaskID == subtask.ID_Subtask).FirstOrDefault();
                LoadAttachs();
            }
            else
            {
                MizuFlow.Registration.unit.Attachments.Create(attach);
                MizuFlow.Registration.unit.Save();
            }
        }

        private void Btn_Delete_Click(object sender, RoutedEventArgs e)
        {
            MizuFlow.MainPage.ToDoSubItem_Delete(this, id);
        }
        ...
        private void ShowImage_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                MizuFlow.AttachWindow window = new AttachWindow(1, attach.ImagePath)
                {
        ...
        private void Btn_EditSubtask_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                subtask.Subtask_Name = txt_Subtask.Text;
